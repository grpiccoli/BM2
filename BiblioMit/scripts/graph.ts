...
var init = async function () {
    loadDates();
    var cuencadata = fetch('/api/static/json/cuencadata.json')
        .then(r => r.json())
    ...
    var comunadata = fetch('/api/static/json/comunadata.json')
        .then(r => r.json())
        .then(data => data.map(processMapData))
    ...
    if (semaforo) {
        var psmbdata = fetch('/api/static/json/psmbdata.json')
            .then(r => r.json())
            .then(data => data.map(processMapData))
            .then(m => markers = markers.concat(m));
